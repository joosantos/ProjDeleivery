					...
					<TransitionChild
						...
						<div
							...
							<FormAthlete
								...
								<div v-if="!isEdit" class="col-span-12 sm:col-span-4">
									<ToogleInput
										:name="'federationActive'"
										:label="t('form.federationActive')"
										:option-selected="stateM.federationActive"
										:error="''"
										@value-changed="
											(option) => (stateM.federationActive = option)
										" />
								</div>
							</FormAthlete>
						</div>
...
watch(
	...
	(after) => {
		...
		if (props.athlete == null) {
			toast.error(t("error.noAthlete"));
			emit("close");
			return;
		}
...
watch(
	...
	() => {
		...
		stateM.value.insuranceEnd = `31-12-${aux[2]}`;
	}
);
...
async function updateInsurance() {
	...
	if (!isEdit.value) apiData.federation_active = stateM.value.federationActive;

	let promise = isEdit.value
		? authApi.put(`insurances/${props.insuranceId}`, apiData)
		: authApi.post(`athletes/${props.athlete.id}/insurances`, apiData);
...
<i18n>
{
  "en_GB": {
    ...
    "error": {
		...
		"noAthlete": "No Atlhete to insure"
    },
	"none": "None",
    "modalTitle": "Athlete's Insurance Information",
  ...
  "pt_PT": {
    ...
    "error": {
		...
		"noAthlete": "Sem Atleta para assegurar"
    },
	"none": "Nenhum",
    "modalTitle": "Informação do Seguro do Atleta",
